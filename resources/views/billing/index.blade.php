...
<head>
    ...
    <title>MediConnect — Billing</title>
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-value { font-family: 'Playfair Display', serif; font-size: 1.8rem; font-weight: 700; color: var(--ink); line-height: 1; margin-bottom: 6px; }
        ...
        .filter-bar input, .filter-bar select { padding: 9px 14px; border: 1.5px solid var(--border); border-radius: 8px; font-size: 13px; font-family: 'Inter', sans-serif; outline: none; color: var(--ink); transition: border-color 0.2s; }
        ...
        .modal { background: white; border-radius: 14px; padding: 28px; max-width: 500px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.2); }
    ...
    <main class="main">
        ...
        <div class="page-header">
            <div>
                ...
                <p>Create invoices, track payments, and manage financial records.</p>
            </div>
            <button class="btn btn-primary" onclick="document.getElementById('modal-invoice').classList.add('open')">
                + Create Invoice
            </button>
        </div>
        ...
        <div class="stat-grid">
            <div class="stat-card">
                ...
                <div class="stat-sub">From {{ $paidInvoices }} paid invoices</div>
            </div>
            <div class="stat-card warn">
                ...
                <div class="stat-sub warn">{{ $pendingInvoices }} pending invoices</div>
            </div>
            ...
            <div class="stat-card danger">
                ...
                <div class="stat-sub danger">All time</div>
            </div>
        </div>

        <div class="card">
            <form class="filter-bar" method="GET" action="{{ route('billing.index') }}">
                <input type="text" name="search" placeholder="🔍  Search invoice, patient, or service…" value="{{ $search ?? '' }}"/>
                <select name="status">
                ...
                <a href="{{ route('billing.index') }}" class="btn" style="background:#f3f4f6;color:var(--ink-mid);">Clear</a>
            </form>
                            ...
                            <td>
                                @if($invoice->status === 'pending')
                                    <button class="action-btn action-pay"
                                        onclick="openPayModal({{ $invoice->id }}, '{{ $invoice->invoice_number }}', '{{ $invoice->formatted_amount }}')">
                                        Mark Paid
                                    </button>
                                    <form method="POST" action="{{ route('billing.cancel', $invoice) }}" style="display:inline;" onsubmit="return confirm('Cancel this invoice?')">
                                        @csrf @method('PATCH')
                                        <button type="submit" class="action-btn action-cancel">Cancel</button>
                                    </form>
                                @elseif($invoice->status === 'paid')
                                    <span style="font-size:12px;color:#065f46;font-weight:600;">✅ Paid {{ $invoice->paid_at?->format('M d') }}</span>
                                @else
                                    <form method="POST" action="{{ route('billing.destroy', $invoice) }}" style="display:inline;" onsubmit="return confirm('Delete this invoice?')">
                                        @csrf @method('DELETE')
    ...
    <div class="modal">
        ...
        <form method="POST" action="{{ route('billing.store') }}">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-ghost" onclick="document.getElementById('modal-invoice').classList.remove('open')">Cancel</button>
...
<div class="modal-overlay" id="modal-pay">
    <div class="modal">
        ...
        <p style="font-size:14px;color:var(--ink-lite);margin-bottom:16px;">
            Invoice <strong id="pay-invoice-num"></strong> — <strong id="pay-invoice-amount"></strong>
        </p>
        <form method="POST" id="pay-form">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-ghost" onclick="document.getElementById('modal-pay').classList.remove('open')">Cancel</button>
...
<script>
    function openPayModal(id, num, amount) {
        document.getElementById('pay-invoice-num').textContent    = num;
        document.getElementById('pay-invoice-amount').textContent = amount;
        document.getElementById('pay-form').action = '/billing/' + id + '/pay';
